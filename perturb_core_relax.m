function [info] = perturb_core_relax(img_index,N_proj,img_sz,aux,type,info)
...
lxls = norm(zls);
% if uxls < lxls
%     lxls = uxls;%(lxls+uxls)/2;%max(norm(zls)-norm(xls-zls),0);
% end
...
% sNR = norm(xls-zls)^2+uxbin^2-lxls^2; %(squared NR)
% sNR = 2*norm(zls)+uxbin^2-lxls^2;
%
% norm(zls)^2+norm(P)^2-2*norm(xls)*norm(zls)*cos; %
% [-norm(zls)^2+norm(P)^2,3*norm(zls)+norm(P)^2]; %
% sNR = norm(zls)^2+uxbin^2-2*lxls*norm(zls)*sqrt(3.5)/2; %
% heu_par = 0.01+(log(img_sz*info.vari)/log(8)-1)*0.015;

parameter=-1;
normzls2 = norm(zls)^2;
sz_para = log(32)/log(img_sz);
proj_para = 2/N_proj;%log(2)/log(N_proj);
heu_par = (0.02*sz_para*proj_para); % [0,4]
while parameter <= 0
    sNR = (heu_par-1)*normzls2+uxbin^2;
%     sNR = -normzls2+heu_par*normzls2+uxbin^2;
    % new_par = (sNR+norm(zls)^2-uxbin^2)/norm(zls)^2;
    
    parameter = sNR - dot(alpha,alpha);
    
    if parameter<0
        %     warning('parameter<0')
        %     (dot(alpha,alpha)-uxbin^2+normzls2)/normzls2-heu_par
        heu_par = 2*heu_par;
    if heu_par==0
        break
    end
    end
    if abs(heu_par)==Inf ||(heu_par)==NaN
        error('fudeu')
    end
end
